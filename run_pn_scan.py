...
from util import progress_bar, save_model, save_args, cal_loss, CosineLRScheduler, worker_init_fn
from datasets.scanobjectnn_c import ScanObjectNNC
...
def parse_args():
    ...
    parser.add_argument('--weight_decay', type=float, default=0.1, help='decay rate')
...
train_transforms_scan = transforms.Compose(
    [
        data_transformer.PointcloudScaleAndTranslate(scale_low=0.9, scale_high=1.1, translate_range=0),
        data_transformer.PointcloudRotate(),
    ]
)
...
def main():
    ...
    net = create_point_former(num_classes=15)
    criterion = cal_loss
    net = net.to(device)
    if device == 'cuda':
    ...
    printf("model tuning parameters: " + str(tuning_num_params))
    printf("tuning rate: " + str(tuning_num_params / num_params))
    printf("===============================================")
    best_test_acc = 0.
    best_train_acc = 0.
    best_test_acc_avg = 0.
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        test_out = validate(net, test_loader, criterion, args.eps, device, epoch)
...
def train(net, trainloader, optimizer, criterion, eps, device, npoints=1024):
    ...
    for batch_idx, (data, xyz, label) in enumerate(trainloader):
        ...
        # new_data = train_transforms_scan(new_data)
...
def validate(net, testloader, criterion, eps, device, epoch):
    net.eval()
    test_loss = 0
    correct = 0
    total = 0
    ...
    with torch.no_grad():
        for batch_idx, (data, xyz, label) in enumerate(testloader):
            data, xyz, label = data.to(device), xyz.to(device), label.to(device).squeeze()
            logits = net(data.permute(0, 2, 1), xyz, extra=False)
            loss = criterion(logits, label, eps)
    ...
    if epoch == 37:
        ...
        label1 = torch.cat(label_list, dim=0)
        tsne = TSNE(n_components=2, random_state=42, n_iter=3000, angle=0.1, init='pca', early_exaggeration=8, perplexity=30.)
        feature = feature.detach().cpu().numpy()
        y = label1.detach().cpu().numpy()
        ...
        for label in range(15):
            ...
            plt.scatter(x_tsne[indices, 0], x_tsne[indices, 1], s=20, color=plt.cm.tab20(label), label=str(label),
                        alpha=1)
        plt.tight_layout()
