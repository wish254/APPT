...
class PointFormerSeg(nn.Module):
    ...
    def __init__(self,num_classes=40, embed_dim=768, depth=12,
        ...
        self.classifier = nn.Sequential(
            nn.Conv1d(512 + 768 + 768, 256, 1, bias=True),
            nn.BatchNorm1d(256),
            nn.Dropout(0.4),
        ...
        self.reduce_dim = 512
        self.propagation_3 = PointNetFeaturePropagation(in_channel=512 + 256,
    ...
    def forward_features(self, x, cls_label):
        ...
        prompt = (group_input_tokens.permute(0, 2, 1).max(-1)[0] + group_input_tokens.permute(0, 2, 1).mean(-1)).unsqueeze(1)
        ...
        for i in range(len(self.blocks)):
            x = torch.cat((x[:, :i, :], prompt, x[:, -196:, :]), dim=1)
            x = self.blocks[i](x)
            # prompt = (x.max(1)[0] + x.mean(1)).unsqueeze(1)
            if i in fetch_idx:
                feature_list.append(x)
        x = torch.cat((x[:, :12, :], prompt, x[:, -196:, :]), dim=1)
        # B,C,N
        ...
        # print(f_level_0.shape)
        prompt = prompt.permute(0, 2, 1).expand(-1, -1, N)
        f_level_0 = torch.cat([f_level_0, l0_feature, prompt], dim=1)
...
if __name__ == '__main__':
    pass
    import timm
    basic_model = timm.create_model('vit_base_patch16_224_in21k', pretrained=True)
    num_classes = 16
    num_part = 50
    B = 2
    ...
    # print(label.shape)
    model = pointFormerSeg(num_classes=num_part).to('cuda')
