...
from datasets.data_util import PartNormalDataset
...
def train(args, io):
    ...
    model = create_point_former().to(device)
    # io.cprint(str(model))
...
def train_epoch(train_loader, model, opt, scheduler, epoch, num_part, num_classes, io):
    ...
    for batch_id, (points, label, target, norm_plt) in tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0.9):
        ...
        # target: b,n
        point = torch.cat((points, norm_plt), 1)
        seg_pred = model(point, points.permute(0, 2, 1))  # seg_pred: b,n,50
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='APF_Seg')
